...
function DogFormContent({ dogId }: { dogId?: string }) {
  const isEdit = !!dogId
  ...
  const set = (k: keyof DogForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
    ...
    <AppShell>
      ...
      <div className="card max-w-lg">
        ...
        <div className="flex items-center gap-4 mb-5">
          <div className="w-20 h-20 rounded-xl overflow-hidden bg-emerald-50 flex items-center justify-center border-2 border-dashed border-emerald-200 flex-shrink-0">
          ...
          <div>
            ...
            <div className="text-xs text-gray-400 mt-1">JPG, PNG or HEIC</div>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4 mb-4">
          <div><label className="label">Dog Name(s) *</label><input className="input" value={form.dog_name} onChange={set('dog_name')} placeholder="e.g. Toby or Mable & Piper" /></div>
          <div><label className="label">Number of Dogs</label><input className="input" type="number" min="1" value={form.number_of_dogs} onChange={set('number_of_dogs')} /></div>
        </div>

        <div className="grid grid-cols-2 gap-4 mb-4">
          <div><label className="label">Owner Name</label><input className="input" value={form.owner_name} onChange={set('owner_name')} placeholder="e.g. Smith" /></div>
          <div><label className="label">Owner Phone</label><input className="input" value={form.owner_phone} onChange={set('owner_phone')} placeholder="e.g. 617-555-1234" /></div>
        </div>

        <div className="mb-4">
          <label className="label">Default Rate ($/day)</label>
          <input className="input" type="number" value={form.default_rate} onChange={set('default_rate')} />
        </div>

        <div className="mb-4">
          <label className="label">Notes</label>
          <textarea className="input" rows={3} value={form.notes} onChange={set('notes')} placeholder="e.g. Anxious around other dogs, needs extra attention…" />
        </div>

        <div className="flex gap-2">
          <button className="btn btn-primary" onClick={save} disabled={saving}>{saving ? 'Saving…' : 'Save Profile'}</button>
          <button className="btn" onClick={() => router.push('/dogs')}>Cancel</button>
          {isEdit && <button className="btn btn-danger ml-auto" onClick={deleteDog}>Delete</button>}
        </div>
      </div>
    </AppShell>
  )
